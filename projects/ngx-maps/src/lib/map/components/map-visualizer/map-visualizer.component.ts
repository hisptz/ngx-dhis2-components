...
export class MapVisualizerComponent implements OnChanges {
  ...
  private layersBounds: any;
  private basemap: any;
  private _baseLayerLegend: any;
  private _currentLegendSets: any;
  ...
  ngAfterViewInit() {
    const shouldDrawMap = this.initializeMapContainer();
    if (shouldDrawMap) {
      interval(4)
        .pipe(take(1))
        .subscribe(() => {
  ...
  initializeMapContainer() {
    ...
    if (mapConfiguration.id != '') {
      ...
      shouldDrawMap = false;
      console.log('no base map set');
    }
    console.log('should draw map', shouldDrawMap);
    return shouldDrawMap;
  }
  ...
  createLayer(optionsLayer, index) {
    console.log('optionsLayer', optionsLayer);
    if (optionsLayer) {
      const {
        displaySettings,
        id,
  ...
  createBaseLayer(basemap?: string) {
    console.log('base layer', basemap);
